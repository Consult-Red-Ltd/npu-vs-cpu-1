...
def main():
    ...
    config = {"device": "rubik", "delegate": DELEGATE, "save_video": SAVE_VIDEO}
    common.write_env(RESULTS_DIR / "env.json", config, [MODEL_PERSON, MODEL_PPE])

    start_temp = common.read_temp_c()
    common.write_baseline_temp(BASELINE_FILE, start_temp)
    print(f"NPU start temp: {start_temp:.1f} C (saved baseline for CPU-only run)")
